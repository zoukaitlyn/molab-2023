...
func loadBundleAudio2(_ fileName:String) -> AVAudioPlayer? {
    let path = Bundle.main.path(forResource: fileName, ofType:nil)!
    let url = URL(fileURLWithPath: path)
    do {
        return try AVAudioPlayer(contentsOf: url)
    } catch {
        print("loadBundleAudio2 error", error)
    }
    return nil
}
...
struct Alarm2: View {
    ...
    @State private var soundFile = bundleAudio2[0]
...
struct Alarm2_Previews: PreviewProvider {
